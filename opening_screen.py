...
def starting_screen():
    ...
    # Print title.
    text = LARGE_FONT.render("CHESS", False, colors.RED)
...
def set_rectangles():
    ...
    rectangles[DIFFICULTY] = create_small_rects("DIFFICULTY", range(1, 5), default=level,
...
def set_difficulty(rect_clicked, text, rectangles):
    global level
    level = int(text)
    set_rects_color(rectangles[DIFFICULTY], rect_clicked, colors.LIGHT_BLUE, colors.DARK_BLUE)
...
rect_group_to_function[DIFFICULTY] = set_difficulty
